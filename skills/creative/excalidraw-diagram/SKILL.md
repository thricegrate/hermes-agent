...
When the Excalidraw MCP server is available, use these tools for the best experience:

### `read_me`
Call once before your first `create_view` in a conversation. Returns the element format reference with color palettes, examples, and sizing rules. Do NOT call it again after the first time.

### `create_view`
Renders a hand-drawn diagram inline with streaming draw-on animations. Pass an `elements` parameter containing a JSON array string of Excalidraw elements.

Key MCP element features (different from raw JSON):
- **Labeled shapes**: Add `label: { text, fontSize }` directly to rectangles/ellipses/diamonds instead of separate text elements
- **cameraUpdate**: Pseudo-element that controls viewport. Use multiple camera positions to guide attention as the diagram streams in. Must use 4:3 aspect ratio (800x600 default)
- **delete**: Pseudo-element `{ type: "delete", ids: "id1,id2" }` to remove elements for transformation animations
- **restoreCheckpoint**: Resume from a previous diagram state using the `checkpointId` returned by prior `create_view` calls
- **Drawing order matters**: Emit progressively (bg -> shape -> label -> arrow -> next shape), not by type

### `export_to_excalidraw`
Uploads the diagram to excalidraw.com and returns a shareable URL. Pass the full serialized Excalidraw JSON as the `json` parameter.

### MCP Workflow
1. Call `read_me` (first time only)
2. Plan layout using the design principles below
3. Call `create_view` with elements array - start with a `cameraUpdate`, then emit elements progressively
4. If the user wants a shareable link, call `export_to_excalidraw`
5. For edits, use `restoreCheckpoint` + `delete` + new elements

### Fallback
If the MCP server is unavailable, fall back to the manual JSON workflow below (generate full Excalidraw JSON, save as `.excalidraw` file, user opens on excalidraw.com).
...
4. Offer the `export_to_excalidraw` MCP tool to generate a shareable excalidraw.com link
5. Tell the user how to use the file:

> **How to view and edit your diagram:**
> - If using MCP: the diagram renders inline. Use `export_to_excalidraw` for a shareable link
