...
**Output the design portion as DESIGN.md.** When the analysis is for design reference (not just CRO critique), structure the design findings into a `DESIGN.md` file using `skills/design-md/templates/design-md-from-brand-analysis.md`. This makes the analysis reusable as a tokens source for Stitch or as inspiration for a differentiated variant. See `skills/design-md/SKILL.md`.
...
2. Analyze the creative (ads-creative)
