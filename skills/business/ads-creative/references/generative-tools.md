...
| Product mockups and variations | Image generation + references | Nano Banana 2 (product refs), Flux (multi-image) |
...
- **Product image reference via `inlineData`** -- pass real product photos alongside text prompts so the model matches actual packaging
...
- **Batch generation with product reference** -- pass 1-3 product photos so generated ads match real packaging (see `ads-designer` Mode 3)

**Product image reference (the key technique):**

Pass product photos as PIL Images in the `contents` array before the text prompt. The model sees your real product and reproduces it in the output. No special "edit endpoint" needed -- this is the standard `generateContent` call with mixed content types.
...
product_photo = Image.open("product-front.png")

response = client.models.generate_content(
    model="gemini-3-pro-image-preview",
    contents=[
        product_photo,  # Reference image -- model matches this product
        "IMAGE 1: Reference photo of the actual product. Match this exactly.\n\n"
        "Create a static ad with a warm cream background. Product centered..."
    ],
...
**Batch generation script:** `ads-designer/scripts/generate_ads.py` automates this for all 40 templates. See `ads-designer` SKILL.md Mode 3.

**API example (text-only):**
```bash
...
**CLI (Python SDK scripts):**
```bash
# Generate voiceover from text
python skills/elevenlabs/scripts/text_to_speech.py --text "Stop wasting hours on manual reporting." --voice Adam --output .tmp/voiceover.mp3

# Clone a brand voice
python skills/elevenlabs/scripts/voice_clone.py --name "Brand Voice" --files sample.mp3

# List available voices
python skills/elevenlabs/scripts/list_voices.py --gender male --accent american
```

See `skills/elevenlabs/SKILL.md` for the full script reference.
