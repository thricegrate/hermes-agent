...
### Protecting Payment/Subscription Fields

Users must never be able to modify their own subscription status, credits, or payment fields. This requires explicit field-level protection in security rules:

```javascript
// Firestore: prevent users from modifying subscription/payment fields
match /users/{userId} {
  allow read: if request.auth.uid == userId;

  // Users can update profile fields, but NOT payment/subscription fields
  allow update: if request.auth.uid == userId
    && !request.resource.data.diff(resource.data).affectedKeys()
        .hasAny(['is_pro', 'plan', 'subscription_end', 'subscription_start',
                 'stripe_customer_id', 'credits', 'tokens_remaining',
                 'is_admin', 'role']);

  // Only Cloud Functions (admin SDK) can modify subscription fields
  // Cloud Functions bypass security rules entirely
}

// Usage counters: users can read but NEVER write
match /usage/{userId} {
  allow read: if request.auth.uid == userId;
...
Without this rule, any authenticated user can open the browser console (web app) or use the Firebase REST API and run:
```javascript
// Attack: grant yourself premium access
firebase.firestore().doc('users/myUserId').update({
  is_pro: true,
  plan: 'premium',
...
### What to check for (Firebase-specific):
- Can users modify their own `is_pro`, `plan`, `credits`, or `tokens_remaining` fields?
- Are usage/quota counters writable only by Cloud Functions?
- Is `request.resource.data.diff(resource.data).affectedKeys()` used to restrict field-level writes?
- Are subcollections (like `users/{uid}/purchases`) properly secured with their own rules?
- Does the project even HAVE a `firestore.rules` file? (if missing, default rules may allow unrestricted access)
