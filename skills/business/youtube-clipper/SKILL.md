...
source: op7418-youtube-clipper-skill (MIT)
source_url: https://github.com/op7418/Youtube-clipper-skill
---

## Adapted for cyber-corsairs

- **Config path:** `.env` lives at `skills/youtube-clipper/.env` (gitignored), not `~/.claude/skills/youtube-clipper/.env` as upstream installer assumes.
- **Install deps before first use:** yt-dlp, FFmpeg with libass support, pysrt, python-dotenv, Python 3.8+. See `scripts/install.sh` or the bottom of this SKILL.md.
- **Do not run upstream installer as part of any automation:** it expects a different config path.
